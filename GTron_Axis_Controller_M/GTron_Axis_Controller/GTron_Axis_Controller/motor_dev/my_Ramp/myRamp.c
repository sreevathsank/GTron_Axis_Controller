...
void find_S_Ramp_Case(rampParams *r_Params)
{
	...
	(r_Params->distance_total > r_Params->s_S_min) ? (DBG_Printf(ERR_LVL_DEBUG, "\nS > S_min\n")) : (DBG_Printf(ERR_LVL_DEBUG, "\nS < S_min\n"));
	(r_Params->distance_total == r_Params->s_S_min) ? (DBG_Printf(ERR_LVL_DEBUG, "\nS = S_min\n")) : (0);
...
void calculate_S_Time(rampParams *r_Params)
{
	...
	{
		case A:  DBG_Printf(ERR_LVL_DEBUG, "\nCase A\n");  break;
		case B:  DBG_Printf(ERR_LVL_DEBUG, "\nCase B\n");  break;
		case C1: DBG_Printf(ERR_LVL_DEBUG, "\nCase C1\n"); break;
		case C2: DBG_Printf(ERR_LVL_DEBUG, "\nCase C2\n"); break;
		case D1: DBG_Printf(ERR_LVL_DEBUG, "\nCase D1\n"); break;
		case D2: DBG_Printf(ERR_LVL_DEBUG, "\nCase D2\n"); break;
		default: break;
	}
	switch(r_Params->ramp_case)
	{
...
void calculate_S_velocities(rampParams *r_Params)
{
	...
	(r_Params->s_v3 != r_Params->target_Velocity) ? DBG_Printf(ERR_LVL_DEBUG, "\nV3 != Vmax\n"): (0);
